...
$year_month = $_GET['historyRecord'];
$year = substr($year_month, 0, 4);
$month = substr($year_month, 4, 2);
$current_month_thai = $thai_months[(int)$month];


$get_record = $con->query("SELECT * FROM `tblrecord` WHERE EXTRACT(YEAR_MONTH FROM `created`) = '" . $year_month . "' AND record_status = 'แจ้งชำระเงินแล้ว' AND record_payment_status = 'ยืนยันแล้ว'");
?>
                ...
                <div class="col-xs-12">
                    <div class="card" style="border: 1px solid #ddd; padding: 10px 30px; border-radius: 5px ;">
                        <div class="card-header">
                            <?php
                            $date = $_GET['historyRecord'];
                            $year = substr($date, 0, 4);
                            $month = substr($date, 4, 2);
                            ?>
                            <h3>ประวัติของเดือน : <?= $current_month_thai ?> ปี : <?= $year ?></h3>
                        </div>
                        <div class="card-body">
