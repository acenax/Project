...
if (isset($_POST['payment_chk'])) {
    ...
    $chkpayment = $con->query("SELECT `bill_trx`, SUM(`price`) As total ,`fname`,`lname`,`phone`, `record_status`, `shipping_rate` FROM `tblrecord` WHERE `bill_trx` = '$paymentid' GROUP BY `bill_trx`");
    if ($chkpayment->num_rows > 0) {
        while ($rows_payment = $chkpayment->fetch_assoc()) {
            ...
            if ($rows_payment['record_status'] != 'ยังไม่ได้แจ้งชำระเงิน') {
                ...
                $totalprice = 0;
                $totalprice = (float)$rows_payment['total'] + (float)$rows_payment['shipping_rate'];
                echo alert_msg(
                    "success",
                    array(
...
if ($ACTION == 'confirmPayment') {
    ...
    // when payment method is bank account transfer
    if (!empty($_POST['bankaccountid'])) {
        $payment_method = 'bank_account_transfer';

        $rs = $con->query("SELECT * FROM `tblbank` WHERE (`bank_id` = '{$_POST['bankaccountid']}') LIMIT 1;");
        if (0 == $rs->num_rows) {
            echo alert_msg("empty", "empty");
            exit();
        }

        $bank_account = $rs->fetch_assoc();
        ...
        if ($isUploaded2) {
            ...
            $note = (!empty($_POST['note'])) ? $_POST['note'] : 'ไม่ได้ระบุ';
            $fullname = $_POST['fname'] . ' ' . $_POST['lname'];
            $insert_payment = $con->query("INSERT INTO `tblpayment`(`payment_method`, `payment_provider`, `payment_account_id`, `payment_account_name`, `payment_fullname`, `payment_phone`, `payment_qty`, `payment_date`, `payment_time`, `payment_pic`, `payment_note`, `bill_trx`) 
            VALUES ('{$payment_method}', '{$bank_account['bank_bank']}', '{$bank_account['bank_number']}', '{$bank_account['bank_name']}', '$fullname','" . $_POST['phone'] . "','" . $_POST['price'] . "','" . $_POST['date'] . "','" . $_POST['time'] . "','$newname','$note','" . $_POST['paymentid'] . "')");
